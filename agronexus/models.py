...
class Usuario(AbstractUser):
    """
    Usuário do sistema com perfis diferenciados
    """
    PERFIL_CHOICES = [
...
class Propriedade(models.Model):
    ...
    coordenadas_gps = models.JSONField(blank=True, null=True, help_text="Coordenadas geográficas da propriedade")
    ...
    def get_taxa_ocupacao_global(self):
        ...
        total_ua = sum(lote.get_total_ua() for lote in self.lotes.filter(ativo=True))
...
class Area(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    propriedade = models.ForeignKey(Propriedade, on_delete=models.CASCADE, related_name='areas')
    nome = models.CharField(max_length=100)
    tipo = models.CharField(max_length=20, choices=TIPO_CHOICES, default='piquete')
    tamanho_ha = models.DecimalField(max_digits=8, decimal_places=2, validators=[MinValueValidator(0)])
    tipo_forragem = models.CharField(max_length=100, blank=True)
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='disponivel')
    coordenadas_poligono = models.JSONField(blank=True, null=True, help_text="Coordenadas do polígono da área")
    ...
    def get_periodo_ocupacao_atual(self):
        ...
        historico = self.historico_ocupacao.filter(data_saida__isnull=True).first()
        if historico:
...
class Animal(models.Model):
    ...
    identificacao_unica = models.CharField(max_length=50, help_text="Brinco, chip, etc.")
    ...
    # Lote atual
    lote_atual = models.ForeignKey('Lote', on_delete=models.SET_NULL, null=True, blank=True, related_name='animais')
    
    # Metadados
    fotos_evolucao = models.JSONField(default=list, blank=True, help_text="Lista de URLs das fotos com datas")
    observacoes = models.TextField(blank=True)
    data_criacao = models.DateTimeField(auto_now_add=True)
    ...
    def get_gmd_periodo(self, dias=30):
        ...
        pesagens = self.pesagens.filter(data_pesagem__gte=data_limite).order_by('data_pesagem')
...
class Lote(models.Model):
    ...
    """
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    propriedade = models.ForeignKey(Propriedade, on_delete=models.CASCADE, related_name='lotes')
    nome = models.CharField(max_length=100)
    descricao = models.TextField(blank=True)
    criterio_agrupamento = models.CharField(max_length=200, help_text="Ex: Bezerros desmamados em 2025")
    area_atual = models.ForeignKey(Area, on_delete=models.SET_NULL, null=True, blank=True, related_name='lotes')
    ativo = models.BooleanField(default=True)
    data_criacao = models.DateTimeField(auto_now_add=True)
    ...
    def get_peso_medio(self):
        ...
        animais_com_peso = [animal for animal in self.animais.filter(status='ativo') if animal.get_peso_atual()]
        if animais_com_peso:
    ...
    def get_gmd_medio(self, dias=30):
        ...
        gmds = [animal.get_gmd_periodo(dias) for animal in self.animais.filter(status='ativo')]
...
class HistoricoLoteAnimal(models.Model):
    ...
    """
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    animal = models.ForeignKey(Animal, on_delete=models.CASCADE, related_name='historico_lotes')
    lote = models.ForeignKey(Lote, on_delete=models.CASCADE, related_name='historico_animais')
...
class HistoricoOcupacaoArea(models.Model):
    ...
    """
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    lote = models.ForeignKey(Lote, on_delete=models.CASCADE, related_name='historico_areas')
    area = models.ForeignKey(Area, on_delete=models.CASCADE, related_name='historico_ocupacao')
    data_entrada = models.DateField()
    data_saida = models.DateField(blank=True, null=True)
    motivo_movimentacao = models.CharField(max_length=200, blank=True)
    usuario = models.ForeignKey(Usuario, on_delete=models.SET_NULL, null=True)
...
class Manejo(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    propriedade = models.ForeignKey(Propriedade, on_delete=models.CASCADE, related_name='manejos')
    tipo = models.CharField(max_length=20, choices=TIPO_CHOICES)
    data_manejo = models.DateField()
    animais = models.ManyToManyField(Animal, related_name='manejos', through='AnimalManejo')
    lote = models.ForeignKey(Lote, on_delete=models.SET_NULL, null=True, blank=True, related_name='manejos')
    
    # Custos
    custo_material = models.DecimalField(max_digits=10, decimal_places=2, default=0)
    custo_pessoal = models.DecimalField(max_digits=10, decimal_places=2, default=0)
...
class Pesagem(models.Model):
    ...
    manejo = models.OneToOneField(Manejo, on_delete=models.CASCADE, related_name='pesagem')
    peso_kg = models.DecimalField(max_digits=6, decimal_places=2, validators=[MinValueValidator(0)])
    ...
    def get_gmd_anterior(self):
        ...
        if pesagem_anterior:
            ...
            diferenca_dias = (self.data_pesagem - pesagem_anterior.data_pesagem).days
            if diferenca_dias > 0:
                return diferenca_peso / diferenca_dias
        return None
...
class EstacaoMonta(models.Model):
    ...
    """
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    propriedade = models.ForeignKey(Propriedade, on_delete=models.CASCADE, related_name='estacoes_monta')
    nome = models.CharField(max_length=100)
    data_inicio = models.DateField()
    data_fim = models.DateField()
    lotes_participantes = models.ManyToManyField(Lote, related_name='estacoes_monta')
    observacoes = models.TextField(blank=True)
    ativa = models.BooleanField(default=True)
...
class ProtocoloIATF(models.Model):
    ...
    """
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    propriedade = models.ForeignKey(Propriedade, on_delete=models.CASCADE, related_name='protocolos_iatf')
    nome = models.CharField(max_length=100)
...
class Inseminacao(models.Model):
    ...
    manejo = models.OneToOneField(Manejo, on_delete=models.CASCADE, related_name='inseminacao')
    data_inseminacao = models.DateField()
    tipo = models.CharField(max_length=10, choices=TIPO_CHOICES)
    reprodutor = models.ForeignKey(Animal, on_delete=models.SET_NULL, null=True, blank=True, related_name='cobertura_realizadas')
    semen_utilizado = models.CharField(max_length=200, blank=True, help_text="Identificação do sêmen")
    protocolo_iatf = models.ForeignKey(ProtocoloIATF, on_delete=models.SET_NULL, null=True, blank=True)
    estacao_monta = models.ForeignKey(EstacaoMonta, on_delete=models.SET_NULL, null=True, blank=True)
    observacoes = models.TextField(blank=True)
...
class DiagnosticoGestacao(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    inseminacao = models.ForeignKey(Inseminacao, on_delete=models.CASCADE, related_name='diagnosticos')
    manejo = models.OneToOneField(Manejo, on_delete=models.CASCADE, related_name='diagnostico_gestacao')
    data_diagnostico = models.DateField()
    resultado = models.CharField(max_length=15, choices=RESULTADO_CHOICES)
    metodo = models.CharField(max_length=100, blank=True, help_text="Ultrassom, palpação, etc.")
    observacoes = models.TextField(blank=True)
...
class Parto(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    mae = models.ForeignKey(Animal, on_delete=models.CASCADE, related_name='partos')
    manejo = models.OneToOneField(Manejo, on_delete=models.CASCADE, related_name='parto')
    data_parto = models.DateField()
    resultado = models.CharField(max_length=15, choices=RESULTADO_CHOICES)
    dificuldade = models.CharField(max_length=15, choices=DIFICULDADE_CHOICES, default='normal')
    bezerro = models.ForeignKey(Animal, on_delete=models.SET_NULL, null=True, blank=True, related_name='parto_origem')
    peso_nascimento = models.DecimalField(max_digits=5, decimal_places=2, blank=True, null=True)
    observacoes = models.TextField(blank=True)
...
class Vacina(models.Model):
    ...
    via_aplicacao = models.CharField(max_length=50, help_text="Subcutânea, intramuscular, etc.")
...
class Vacinacao(models.Model):
    ...
    """
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    manejo = models.OneToOneField(Manejo, on_delete=models.CASCADE, related_name='vacinacao')
...
class AdministracaoMedicamento(models.Model):
    ...
    """
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    manejo = models.OneToOneField(Manejo, on_delete=models.CASCADE, related_name='administracao_medicamento')
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.data_fim_carencia:
            self.data_fim_carencia = self.manejo.data_manejo + timedelta(days=self.medicamento.periodo_carencia_dias)
        super().save(*args, **kwargs)
...
class CalendarioSanitario(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    propriedade = models.ForeignKey(Propriedade, on_delete=models.CASCADE, related_name='calendario_sanitario')
    ...
    vacina = models.ForeignKey(Vacina, on_delete=models.SET_NULL, null=True, blank=True)
    ...
    manejo_realizado = models.ForeignKey(Manejo, on_delete=models.SET_NULL, null=True, blank=True)
    usuario = models.ForeignKey(Usuario, on_delete=models.SET_NULL, null=True)
    data_criacao = models.DateTimeField(auto_now_add=True)
...
class ContaFinanceira(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    propriedade = models.ForeignKey(Propriedade, on_delete=models.CASCADE, related_name='contas_financeiras')
    nome = models.CharField(max_length=100)
    tipo = models.CharField(max_length=20, choices=TIPO_CHOICES)
    ...
    def get_saldo_atual(self):
        ...
        entradas = self.lancamentos_entrada.aggregate(total=models.Sum('valor'))['total'] or 0
        saidas = self.lancamentos_saida.aggregate(total=models.Sum('valor'))['total'] or 0
...
class CategoriaFinanceira(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    propriedade = models.ForeignKey(Propriedade, on_delete=models.CASCADE, related_name='categorias_financeiras')
    nome = models.CharField(max_length=100)
    tipo = models.CharField(max_length=10, choices=TIPO_CHOICES)
    descricao = models.TextField(blank=True)
    ativa = models.BooleanField(default=True)
...
class LancamentoFinanceiro(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    propriedade = models.ForeignKey(Propriedade, on_delete=models.CASCADE, related_name='lancamentos_financeiros')
    ...
    # Contas envolvidas
    conta_origem = models.ForeignKey(ContaFinanceira, on_delete=models.CASCADE, related_name='lancamentos_saida')
    conta_destino = models.ForeignKey(ContaFinanceira, on_delete=models.SET_NULL, null=True, blank=True, related_name='lancamentos_entrada')
    
    # Relacionamentos com outras entidades
    manejo_relacionado = models.ForeignKey(Manejo, on_delete=models.SET_NULL, null=True, blank=True, related_name='lancamentos_financeiros')
    animal_relacionado = models.ForeignKey(Animal, on_delete=models.SET_NULL, null=True, blank=True, related_name='lancamentos_financeiros')
    ...
    def clean(self):
        ...
        if self.tipo != 'transferencia' and self.conta_destino:
            raise ValidationError('Apenas transferências podem ter conta de destino')
...
class RelatorioPersonalizado(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    propriedade = models.ForeignKey(Propriedade, on_delete=models.CASCADE, related_name='relatorios')
...
class ConfiguracaoSistema(models.Model):
    ...
    """
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    propriedade = models.OneToOneField(Propriedade, on_delete=models.CASCADE, related_name='configuracao')
    
    # Configurações de peso e medidas
    peso_ua_referencia = models.DecimalField(max_digits=6, decimal_places=2, default=450, help_text="Peso de referência para 1 UA em kg")
