...
from .models import (Animal, CalendarioSanitario, ConfiguracaoSistema,
                     LancamentoFinanceiro, Manejo, Pesagem, Propriedade,
                     Usuario)
from .utils.helpers import (create_backup_filename, export_to_excel,
                            generate_report_data, serialize_for_backup)
...
def verificar_calendario_sanitario(self):
    ...
    try:
        ...
        logger.info(
            f"Verificação de calendário concluída. {total_notificacoes} notificações enviadas.")
...
def enviar_notificacao_calendario(self, propriedade_id, itens_ids):
    ...
    try:
        ...
        logger.info(
            f"Notificação enviada para {propriedade.proprietario.email}")
...
def gerar_relatorio_semanal(self):
    ...
    try:
        ...
        for propriedade in propriedades:
            relatorio = gerar_relatorio_propriedade(
                propriedade, semana_passada, hoje)
...
def calcular_indices_zootecnicos(self, propriedade_id):
    ...
    try:
        ...
        if total_femeas > 0:
            indices['indices']['taxa_natalidade'] = (
                nascimentos_ano / total_femeas) * 100
        ...
        if total_animais > 0:
            indices['indices']['taxa_mortalidade'] = (
                mortes_ano / total_animais) * 100
