...
class PropriedadeFilter(django_filters.FilterSet):
    ...
    area_total_min = django_filters.NumberFilter(
        field_name='area_total_ha', lookup_expr='gte')
    area_total_max = django_filters.NumberFilter(
        field_name='area_total_ha', lookup_expr='lte')
...
class AreaFilter(django_filters.FilterSet):
    ...
    tamanho_min = django_filters.NumberFilter(
        field_name='tamanho_ha', lookup_expr='gte')
    tamanho_max = django_filters.NumberFilter(
        field_name='tamanho_ha', lookup_expr='lte')
...
class AnimalFilter(django_filters.FilterSet):
    ...
    identificacao = django_filters.CharFilter(
        field_name='identificacao_unica', lookup_expr='icontains')
    nome = django_filters.CharFilter(
        field_name='nome_registro', lookup_expr='icontains')
    ...
    # Filtros por idade
    idade_min_meses = django_filters.NumberFilter(
        method='filter_idade_min_meses')
    idade_max_meses = django_filters.NumberFilter(
        method='filter_idade_max_meses')

    # Filtros por data
    nascimento_inicio = django_filters.DateFilter(
        field_name='data_nascimento', lookup_expr='gte')
    nascimento_fim = django_filters.DateFilter(
        field_name='data_nascimento', lookup_expr='lte')

    # Filtros por valor
    valor_compra_min = django_filters.NumberFilter(
        field_name='valor_compra', lookup_expr='gte')
    valor_compra_max = django_filters.NumberFilter(
        field_name='valor_compra', lookup_expr='lte')
...
class LoteFilter(django_filters.FilterSet):
    ...
    criterio = django_filters.CharFilter(
        field_name='criterio_agrupamento', lookup_expr='icontains')
    ativo = django_filters.BooleanFilter()
    area = django_filters.ModelChoiceFilter(
        field_name='area_atual', queryset=Area.objects.all())
...
class ManejoFilter(django_filters.FilterSet):
    ...
    data_inicio = django_filters.DateFilter(
        field_name='data_manejo', lookup_expr='gte')
    data_fim = django_filters.DateFilter(
        field_name='data_manejo', lookup_expr='lte')

    # Filtros por custo
    custo_min = django_filters.NumberFilter(
        field_name='custo_total', lookup_expr='gte')
    custo_max = django_filters.NumberFilter(
        field_name='custo_total', lookup_expr='lte')

    # Filtros relacionados
    lote = django_filters.ModelChoiceFilter(queryset=Lote.objects.all())
    usuario = django_filters.CharFilter(
        field_name='usuario__username', lookup_expr='icontains')
...
class PesagemFilter(django_filters.FilterSet):
    ...
    data_inicio = django_filters.DateFilter(
        field_name='data_pesagem', lookup_expr='gte')
    data_fim = django_filters.DateFilter(
        field_name='data_pesagem', lookup_expr='lte')

    # Filtros por peso
    peso_min = django_filters.NumberFilter(
        field_name='peso_kg', lookup_expr='gte')
    peso_max = django_filters.NumberFilter(
        field_name='peso_kg', lookup_expr='lte')

    # Filtros relacionados
    animal = django_filters.ModelChoiceFilter(queryset=Animal.objects.all())
    equipamento = django_filters.CharFilter(
        field_name='equipamento_usado', lookup_expr='icontains')
...
class EstacaoMontaFilter(django_filters.FilterSet):
    ...
    # Filtros por data
    inicio_periodo = django_filters.DateFilter(
        field_name='data_inicio', lookup_expr='gte')
    fim_periodo = django_filters.DateFilter(
...
class InseminacaoFilter(django_filters.FilterSet):
    ...
    data_inicio = django_filters.DateFilter(
        field_name='data_inseminacao', lookup_expr='gte')
    data_fim = django_filters.DateFilter(
        field_name='data_inseminacao', lookup_expr='lte')

    # Filtros relacionados
    animal = django_filters.ModelChoiceFilter(queryset=Animal.objects.all())
    reprodutor = django_filters.ModelChoiceFilter(
        queryset=Animal.objects.all())
    estacao_monta = django_filters.ModelChoiceFilter(
        queryset=EstacaoMonta.objects.all())
    protocolo_iatf = django_filters.ModelChoiceFilter(
        queryset=ProtocoloIATF.objects.all())

    # Filtro por sêmen
    semen = django_filters.CharFilter(
        field_name='semen_utilizado', lookup_expr='icontains')

    class Meta:
        model = Inseminacao
        fields = ['animal', 'tipo', 'reprodutor',
                  'estacao_monta', 'protocolo_iatf']


class DiagnosticoGestacaoFilter(django_filters.FilterSet):
    """Filtro para diagnósticos de gestação"""
    resultado = django_filters.ChoiceFilter(
        choices=DiagnosticoGestacao.RESULTADO_CHOICES)
    data_inicio = django_filters.DateFilter(
        field_name='data_diagnostico', lookup_expr='gte')
    data_fim = django_filters.DateFilter(
        field_name='data_diagnostico', lookup_expr='lte')
    metodo = django_filters.CharFilter(lookup_expr='icontains')

    # Filtros relacionados
    animal = django_filters.ModelChoiceFilter(
        field_name='inseminacao__animal', queryset=Animal.objects.all())
    estacao_monta = django_filters.ModelChoiceFilter(
        field_name='inseminacao__estacao_monta', queryset=EstacaoMonta.objects.all())
...
class PartoFilter(django_filters.FilterSet):
    ...
    dificuldade = django_filters.ChoiceFilter(
        choices=Parto.DIFICULDADE_CHOICES)
    data_inicio = django_filters.DateFilter(
        field_name='data_parto', lookup_expr='gte')
    data_fim = django_filters.DateFilter(
        field_name='data_parto', lookup_expr='lte')

    # Filtros por peso
    peso_min = django_filters.NumberFilter(
        field_name='peso_nascimento', lookup_expr='gte')
    peso_max = django_filters.NumberFilter(
        field_name='peso_nascimento', lookup_expr='lte')
...
class VacinacaoFilter(django_filters.FilterSet):
    ...
    data_inicio = django_filters.DateFilter(
        field_name='manejo__data_manejo', lookup_expr='gte')
    data_fim = django_filters.DateFilter(
        field_name='manejo__data_manejo', lookup_expr='lte')
    ...
    proxima_dose = django_filters.DateFilter(
        field_name='data_proxima_dose', lookup_expr='lte')
...
class CalendarioSanitarioFilter(django_filters.FilterSet):
    ...
    status = django_filters.ChoiceFilter(
        choices=CalendarioSanitario.STATUS_CHOICES)
    tipo_manejo = django_filters.ChoiceFilter(choices=Manejo.TIPO_CHOICES)

    # Filtros por data
    data_inicio = django_filters.DateFilter(
        field_name='data_agendada', lookup_expr='gte')
    data_fim = django_filters.DateFilter(
        field_name='data_agendada', lookup_expr='lte')
    ...
    # Filtros especiais
    vencimentos_proximos = django_filters.NumberFilter(
        method='filter_vencimentos_proximos')

    class Meta:
        model = CalendarioSanitario
        fields = ['propriedade', 'status',
                  'tipo_manejo', 'vacina', 'medicamento']
...
class LancamentoFinanceiroFilter(django_filters.FilterSet):
    ...
    tipo = django_filters.ChoiceFilter(
        choices=LancamentoFinanceiro.TIPO_CHOICES)
    data_inicio = django_filters.DateFilter(
        field_name='data_lancamento', lookup_expr='gte')
    data_fim = django_filters.DateFilter(
        field_name='data_lancamento', lookup_expr='lte')
    ...
    conta_origem = django_filters.ModelChoiceFilter(
        queryset=ContaFinanceira.objects.all())
    conta_destino = django_filters.ModelChoiceFilter(
        queryset=ContaFinanceira.objects.all())
    ...
    class Meta:
        ...
        fields = ['propriedade', 'tipo', 'categoria',
...
class RelatorioRebanhoFilter(django_filters.FilterSet):
    ...
    # Filtros básicos
    propriedade = django_filters.ModelChoiceFilter(
    ...
    lote = django_filters.ModelChoiceFilter(
        field_name='lote_atual', queryset=Lote.objects.all())
    area = django_filters.ModelChoiceFilter(
        field_name='lote_atual__area_atual', queryset=Area.objects.all())
